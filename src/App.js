...
function App() {
  ...
  async function getWeather() {
    const response = await fetch(
      `https://api.openweathermap.org/data/2.5/forecast/daily?q=${chosenCity}&units=${chosenUnit}&cnt=7&appid=08e194dbf106b4a880235ddea75704dd`
    );
  ...
  return (
    <div className="container">
      ...
      <div className="sevenDat">
        {dayList.map((dayTest) => (
          <SevenDayForecast
            className="daySpacing"
            sevenDayTemp={dayTest.temp.day}
            sevenDayWeather={dayTest.weather[0].main}
            sevenDayIcon={dayTest.weather[0].icon}
          />
        ))}
      </div>
    </div>
